...
extern "C" {
    ...
    JNIEXPORT bool JNICALL Java_com_mapzen_tangram_MapController_nativeLongitudeLatitudeToScreenPosition(JNIEnv* jniEnv, jobject obj, jlong mapPtr, jdouble lon, jdouble lat, jfloatArray screenPosition) {
        assert(mapPtr > 0);
        auto map = reinterpret_cast<Tangram::Map*>(mapPtr);
        jfloat* arr = jniEnv->GetFloatArrayElements(screenPosition, NULL);
        bool offViewport = map->lonLatToScreenPosition(lon, lat, arr[0], arr[1]);
